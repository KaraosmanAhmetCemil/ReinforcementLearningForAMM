...
class AutomatedMarketMaker:
    def __init__(self, token_x_reserve, token_y_reserve, fee_tier=0.003, price_range=None):
        ...
        # Add state tracking
        self.equilibrium_states = []  # Track (x, y, valuation)
        self.trade_history = []  # Track (delta_x, delta_y, div_loss, slip_loss)
    ...
    def calculate_liquidity_utilization(self, trade_volume):
        """
        Computes liquidity utilization metric with dynamic adjustments based on market conditions.
        """
        # Dynamic liquidity adjustment based on volatility and trade volume
        volatility = np.std(self.price_history[-50:]) if len(self.price_history) >= 50 else 0.01
        liquidity_depth = (self.token_x_reserve + self.token_y_reserve) / 2
        
        # Adjust liquidity utilization based on volatility and trade volume
        if volatility < 0.005:  # Low volatility
            liquidity_utilization = trade_volume / (liquidity_depth * 0.8)  # Encourage higher utilization
        else:  # High volatility
            liquidity_utilization = trade_volume / (liquidity_depth * 1.2)  # Conservative utilization
