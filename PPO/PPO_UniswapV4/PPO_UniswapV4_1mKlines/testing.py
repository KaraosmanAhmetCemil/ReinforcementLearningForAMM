...
from utility import select_action_ppo, detect_price_event, compute_expected_reward
import pandas as pd


def get_predictions(lstm_model, ppo_network, test_data, test_alternative_data, device=None, seq_length=50, amm=None):
    """
    Generate predictions from the LSTM and select actions using the PPO network.
    During evaluation, we choose actions deterministically.
    """
    lstm_model.eval()
    ppo_network.eval()
    ...
    with torch.no_grad():
        ...
        for i in range(len(test_predictions)):
            ...
            action, _, _ = select_action_ppo(state, ppo_network, device, deterministic=True)
...
def evaluation(test_data, test_data_scaled, test_alternative_data_scaled, 
               lstm_model, ppo_network, main_scaler, alt_scaler, seq_length, device=None):
    ...
    print(f"Total Fees: {viz_data['metrics']['total_fees']:.6f}")
    print(f"Hook Revenue: {viz_data['metrics']['total_hook_revenue']:.6f}")
    print(f"Divergence Loss: {viz_data['metrics']['total_divergence']:.6f}")
    print(f"Slippage Loss: {viz_data['metrics']['total_slippage']:.6f}")
    print(f"MEV Risk: {viz_data['metrics']['total_mev_risk']:.6f}")
    print(f"Gas Costs: {viz_data['metrics']['total_gas_cost']}")
    print(f"Average Liquidity Utilization: {viz_data['metrics']['average_liquidity_utilization']:.6%}")
